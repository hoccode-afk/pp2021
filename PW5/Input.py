...
# Function to import informations from marks.txt
def input_file(driver, mark, sid, sname, sdob, cid, cname, credit, value):
    # Set for students
    driver.mark.student.set_sid(sid)    #int
    driver.mark.student.set_sname(sname)    #str
    driver.mark.student.set_sdob(sdob)   #str

    # Set for courses
    driver.mark.course.set_cid(cid)     #int
    driver.mark.course.set_cname(cname)     #str
    driver.mark.course.set_credit(credit)   #int

    # Set for marks
    driver.mark.set_value(value)    #int
...
class Input:
    ...
    def input_number_students(self):
        ...
        #Writing data to students.txt file
        sf = open("students.txt", "a")
        sf.write(str(self.driver.nofstudents))
        sf.write("\n")
        sf.close()
    ...
    def input_number_courses(self):
        ...
        #Writing data to courses.txt file
        cf = open("courses.txt", "a")
        cf.write(str(self.driver.nofcourses))
        cf.write("\n")
        cf.close()
